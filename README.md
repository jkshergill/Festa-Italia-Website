...
# Our ERD
  <img width="1729" height="1125" alt="Festa Italia ERD Database" src="https://github.com/user-attachments/assets/80884e54-cc3c-4ec0-b363-3f9de6a2daff" />
...
* Admin Dashboard
  ...
  * This is the page an admin will be directed to when clicking the Admin Dashboard button in the buger menu with various admin features.
...
* Fishermans Festival Page
  ...
  * This is a page accessed through the burger menu that gives information to a user about various festival details along with the ability to sign up for volunteering and viewing the food menu.
...
* Volunteer Page
  ...
  * This page is accessed from the Fishermans Festival page and allows users to request volunteering by day, time frame, and booth. This page is only available if a user is signed in.
...
* Coronation Ball Tickets Page
  ...
  * This page is accessed through the Queen's Court Page via the Buy Tickets button. Users must be signed in to buy tickets. This page allows users to buy tickets to the coronation ball via Clover. 
...
-   Node.js (v20 or higher) installed. To check your current version, run"
    '''bash
    node --version
    '''
    If not installed or below v20, downlaod it from [nodejs.org](https://nodejs.org/).
-   npm installed (this comes bundled with Node.js). Verify it is by running:
    '''bash
    npm --version
    '''
...
This project uses npm to manage all JavaScript dependencies. Dependencies are defined in `package.json` at the root of the repository.

**Installing dependencies**

After cloning the repository, always run the following before starting the app or running tests:

```bash
npm install
```

This reads `package.json` and installs all listed packages into the `node_modules/` folder. The `package-lock.json` file locks exact versions so every install is reproducible — do not delete it.

**Adding a new dependency**

```bash
npm install <package-name>          # production dependency
npm install -D <package-name>       # development-only dependency (e.g. testing tools)
```
...
To update all packages within the version ranges specified in `package.json`:

```bash
npm update
```

> **Important:** Never manually edit `package-lock.json`. It is managed automatically by npm.

## Vite Configuration

This project uses [Vite](https://vitejs.dev/) as the build tool and development server. The Vite configuration lives in `vite.config.js` (or `vite.config.ts`) at the root of the project.

A typical configuration for this project looks like:
...
Supabase provides the backend for this project, including authentication, the database, and file storage. Follow these steps to configure and verify your Supabase project.
...
> **Never use the `service_role` key in the frontend.** It bypasses Row Level Security and must only be used in secure server-side environments.

### 2. Configure Environment Variables

Create a `.env` file in the root of the repository (if it does not already exist) and add your credentials:
...
Supabase enables Row Level Security by default. You must define policies for each table to control who can read or write data. At minimum:

- **Public tables** (e.g. `foods`, `events`, `sponsors`): allow `SELECT` for all users (including unauthenticated).
- **User-specific tables** (e.g. `tickets`, `volunteers`, `donations`): allow `SELECT`, `INSERT`, and `UPDATE` only for the authenticated user who owns the row, typically using `auth.uid() = user_id`.
- **Admin-only tables**: restrict write access to users with an admin role. This can be implemented by checking a `role` column on the `users` table.

To enable RLS on a table and add a policy through Supabase, click on "Authentication" from the side menu and scroll to "CONFIGURATION" to navigate to "Policies". From here you can create new policies, edit existing policies, or disable existing policies.

### 4. Storage Buckets

Supabase Storage is used to store images for food items, donors, tokens, and other assets. Set each bucket's visibility to **Public** if images are displayed without authentication, or **Private** if they require a signed URL.

### 5. Authentication

This project uses Supabase Auth for user login and signup. Email/password authentication is enabled by default. To configure it:

1. In the Supabase dashboard, go to **Authentication → Providers**.
2. Ensure **Email** is enabled.
3. Under **Authentication → URL Configuration**, set your **Site URL** to your production domain (e.g. `https://www.festaitalia.org`) and add `http://localhost:5173` to the **Redirect URLs** list for local development.
...
3.  Add the required Supabase environment variables (see [Supabase Setup](#supabase-setup) above).
...
5.  Run tests (also see [Testing](#testing) above for more details):
``` bash
npm run test
```
...
npm install
```
This installs all packages listed in `package.json` into `node_modules/`. Re-run this command any time `package.json` changes (e.g. after pulling updates from the repository that add or remove packages).
...
VITE_SUPABASE_ANON_KEY=your-supabase-anon-key
```
See [Supabase Setup](#supabase-setup) for instructions on where to find these values.
...
The `vite.config.js` file at the root of the project controls both the dev server and the production build. See the [Vite Configuration](#vite-configuration) section above for a full explanation of each option.

Common things you may need to change here:

- `server.port` — if port 5173 conflicts with another local process.
- `test.environment` — must remain `jsdom` for React component tests to work correctly.
...
The Supabase client is initialised once in `src/supabaseClient.js` and imported wherever database or auth calls are needed:
...
``` bash
npm run test [filename].spec.jsx
```
Test files follow the naming pattern `*.spec.jsx` and live alongside the components they test inside the `src/` folder.
...
-   Run `npm install` again to make sure all dependencies are present
-   Check `.env` file exists and contains valid `VITE_SUPABASE_URL` and `VITE_SUPABASE_ANON_KEY` values
- If port 5173 is in use, update `server.port` in `vite.config.js`

### Supabase not working

- Verify API keys match those in the Supabase dashboard under **Project Settings → API**
- Check RLS policies — a missing policy will silently return empty data rather than an error
- Confirm you are referencing the correct table and column names
- Open the Supabase dashboard **Logs → API** tab to inspect failed requests in real time
